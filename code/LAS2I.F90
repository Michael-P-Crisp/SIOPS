...
!c----------------------------------------------------------------------------
      ...
      subroutine las2i( dvarfn, N1, N2, XL, YL, MXM, MXK ,C0, CT, CC, CS, CI, AT, AC, AS, AI, M,k1, k2, kk, iout, tol , var, pb, H, G, da, db,thx,thy )
      integer MXK !=256
      ...
      real(8) da, db 
      real(8) thx,thy !SOF in x and y directions
      external dvarfn
      !character dvarfn
...
!c                    decompose N1 and N2
      ...
      do  m = 0, MXM
         ...
         j1 = k1/2
         j2 = k2/2
         if( 2*j1 .ne. k1 .or. 2*j2 .ne. k2 ) exit
         k1 = j1
         k2 = j2
	  end do
	  if( kk .gt. MXK ) then
		  write(*,*)'Error: unable to determine an acceptable combination of k1, k2 and m'
		  write(*,*)'       such that k1*2**m = N1, k2*2**m = N2.'
		  write(*,*)'       k1 = ',k1,', k2 = ',k2,', m = ',m
		  write(*,*)'       (k1*k2 must be less than ',MXK,' and m must be less than ',MXM,')'
		  write(*,*)'       Try changing N1 and N2.'
		  stop
      end if
...
!c                    form initial covariance matrix
      T1 = dble(XL)/dble(k1)
      T2 = dble(YL)/dble(k2)
      call dcvit2( dvarfn, R0, kk, R, 9, k1, k2, T1, T2 , var, pb, H, G, da, db , thx, thy)
...
!c                    get basic cov matrices
         lformR = (1 .lt. M)
         call dcvmt2(dvarfn,R(1,1,in),9,B,4,S,9,T1,T2,lformR, var, pb, H, G, da, db , thx, thy)
         i2 = 5
         if( k1 .eq. 1 ) then
            i1 = 2
            i3 = 8
