...
!c ==========================================================================
      ...
      subroutine las2g( Z, N1, N2, XL, YL, kseed, iout, &
        C0,CT,CC,CS,CI,AT,AC,AS,AI,M,K1,K2,KK,NN, MXM,MXK,NGS)
      integer MXM,MXK,NGS ! MXM = 9, MXK = 256, NGS = 6912 )
      real Z(*)
      integer init
      real C0((MXK*(MXK + 1))/2), U(NGS)
      real CT(6,2), CC(6,4,MXM), CS(6,4,MXM), CI(6,MXM)
      real AT(3,3,2), AC(4,3,4,MXM), AS(6,3,4,MXM), AI(9,3,MXM)
      real(8) var, pb, H, da, db , thx, thy
      real(8), external :: dvarfn
      data zero/0.0/, four/4.0/
      data tol/1.e-3/
...
!c                    pointers into Z
         ...
         call vnorm( U, 3*jx )
!c                    corner #1
         Z(i0)   = AC(1,1,1,i)*Z(j0) + AC(2,1,1,i)*Z(j0+1) + AC(3,1,1,i)*Z(j1) + AC(4,1,1,i)*Z(j1+1) + CC(1,  1,i)*U(1)
         Z(i0+1) = AC(1,2,1,i)*Z(j0) + AC(2,2,1,i)*Z(j0+1) + AC(3,2,1,i)*Z(j1) + AC(4,2,1,i)*Z(j1+1) + CC(2,  1,i)*U(1) +  CC(3,  1,i)*U(2)
         Z(i1)   = AC(1,3,1,i)*Z(j0) + AC(2,3,1,i)*Z(j0+1) + AC(3,3,1,i)*Z(j1) + AC(4,3,1,i)*Z(j1+1) + CC(4,  1,i)*U(1) +  CC(5,  1,i)*U(2) + CC(6,  1,i)*U(3)
         Z(i1+1) = four*Z(j0) - Z(i0) - Z(i0+1) - Z(i1)
!c                    side #1
         L = 4
         do js = 1, jx-2
            i0 = i0 + 2
            i1 = i1 + 2
            Z(i0)   = AS(1,1,1,i)*Z(j0  ) + AS(2,1,1,i)*Z(j0+1) + AS(3,1,1,i)*Z(j0+2) + AS(4,1,1,i)*Z(j1) &
                + AS(5,1,1,i)*Z(j1+1) + AS(6,1,1,i)*Z(j1+2) + CS(1,  1,i)*U(L)
            Z(i0+1) = AS(1,2,1,i)*Z(j0  ) + AS(2,2,1,i)*Z(j0+1) + AS(3,2,1,i)*Z(j0+2) + AS(4,2,1,i)*Z(j1) &
                + AS(5,2,1,i)*Z(j1+1) + AS(6,2,1,i)*Z(j1+2) + CS(2,  1,i)*U(L) + CS(3,  1,i)*U(L+1)
            Z(i1)   = AS(1,3,1,i)*Z(j0  ) + AS(2,3,1,i)*Z(j0+1) + AS(3,3,1,i)*Z(j0+2) + AS(4,3,1,i)*Z(j1) &
                + AS(5,3,1,i)*Z(j1+1) + AS(6,3,1,i)*Z(j1+2) + CS(4,  1,i)*U(L) + CS(5,  1,i)*U(L+1) + CS(6,  1,i)*U(L+2)
            Z(i1+1) = four*Z(j0+1) - Z(i0) - Z(i0+1) - Z(i1)
            j0 = j0 + 1
            j1 = j1 + 1
            L  = L  + 3
...
!c                    corner #2
         i0 = i0 + 2
         i1 = i1 + 2
         Z(i0)   = AC(1,1,2,i)*Z(j0) + AC(2,1,2,i)*Z(j0+1) + AC(3,1,2,i)*Z(j1) + AC(4,1,2,i)*Z(j1+1) + CC(1,  2,i)*U(L)
         Z(i0+1) = AC(1,2,2,i)*Z(j0) + AC(2,2,2,i)*Z(j0+1) + AC(3,2,2,i)*Z(j1) + AC(4,2,2,i)*Z(j1+1) + CC(2,  2,i)*U(L)  + CC(3,  2,i)*U(L+1)
         Z(i1)   = AC(1,3,2,i)*Z(j0) + AC(2,3,2,i)*Z(j0+1) + AC(3,3,2,i)*Z(j1) + AC(4,3,2,i)*Z(j1+1) + CC(4,  2,i)*U(L)  + CC(5,  2,i)*U(L+1) + CC(6,  2,i)*U(L+2)
         Z(i1+1) = four*Z(j0+1) - Z(i0) - Z(i0+1) - Z(i1)
         ...
         do ks = 1, jy-2
            ...
            call vnorm( U, 3*jx )
!c                    side #2
            Z(i0)   = AS(1,1,2,i)*Z(j0) + AS(2,1,2,i)*Z(j0+1) + AS(3,1,2,i)*Z(j1) + AS(4,1,2,i)*Z(j1+1) + AS(5,1,2,i)*Z(j2) + AS(6,1,2,i)*Z(j2+1) + CS(1,  2,i)*U(1)
            Z(i0+1) = AS(1,2,2,i)*Z(j0) + AS(2,2,2,i)*Z(j0+1) + AS(3,2,2,i)*Z(j1) + AS(4,2,2,i)*Z(j1+1) + AS(5,2,2,i)*Z(j2) + AS(6,2,2,i)*Z(j2+1) &
                    + CS(2,  2,i)*U(1)  + CS(3,  2,i)*U(2)
            Z(i1)   = AS(1,3,2,i)*Z(j0) + AS(2,3,2,i)*Z(j0+1) + AS(3,3,2,i)*Z(j1) + AS(4,3,2,i)*Z(j1+1) + AS(5,3,2,i)*Z(j2) + AS(6,3,2,i)*Z(j2+1) &
                    + CS(4,  2,i)*U(1)  + CS(5,  2,i)*U(2) + CS(6,  2,i)*U(3)
...
!c                    interior
            ...
            do js = 1, jx-2
               ...
               it = i0 + 1
               i1 = i1 + 2
               Z(i0)=AI(1,1,i)*Z(j0)+AI(2,1,i)*Z(j0+1)+AI(3,1,i)*Z(j0+2) +AI(4,1,i)*Z(j1)+AI(5,1,i)*Z(j1+1)+AI(6,1,i)*Z(j1+2)  &
                    +AI(7,1,i)*Z(j2)+AI(8,1,i)*Z(j2+1)+AI(9,1,i)*Z(j2+2)+CI(1,  i)*U(L)
               Z(it)=AI(1,2,i)*Z(j0)+AI(2,2,i)*Z(j0+1)+AI(3,2,i)*Z(j0+2) +AI(4,2,i)*Z(j1)+AI(5,2,i)*Z(j1+1)+AI(6,2,i)*Z(j1+2) &
                    +AI(7,2,i)*Z(j2)+AI(8,2,i)*Z(j2+1)+AI(9,2,i)*Z(j2+2)+CI(2,  i)*U(L) +CI(3,  i)*U(L+1)
               Z(i1)=AI(1,3,i)*Z(j0)+AI(2,3,i)*Z(j0+1)+AI(3,3,i)*Z(j0+2)+AI(4,3,i)*Z(j1)+AI(5,3,i)*Z(j1+1)+AI(6,3,i)*Z(j1+2) &
                    +AI(7,3,i)*Z(j2)+AI(8,3,i)*Z(j2+1)+AI(9,3,i)*Z(j2+2) +CI(4,  i)*U(L) +CI(5,  i)*U(L+1) +CI(6,  i)*U(L+2)
...
!c                    side #3
            i0 = i0 + 2
            i1 = i1 + 2
            Z(i0)   = AS(1,1,3,i)*Z(j0) + AS(2,1,3,i)*Z(j0+1) + AS(3,1,3,i)*Z(j1) + AS(4,1,3,i)*Z(j1+1) + AS(5,1,3,i)*Z(j2) + AS(6,1,3,i)*Z(j2+1) + CS(1,  3,i)*U(L)
            Z(i0+1) = AS(1,2,3,i)*Z(j0) + AS(2,2,3,i)*Z(j0+1) + AS(3,2,3,i)*Z(j1) + AS(4,2,3,i)*Z(j1+1) + AS(5,2,3,i)*Z(j2) + AS(6,2,3,i)*Z(j2+1) &
                    + CS(2,  3,i)*U(L)  + CS(3,  3,i)*U(L+1)
            Z(i1)   = AS(1,3,3,i)*Z(j0) + AS(2,3,3,i)*Z(j0+1) + AS(3,3,3,i)*Z(j1) + AS(4,3,3,i)*Z(j1+1) + AS(5,3,3,i)*Z(j2) + AS(6,3,3,i)*Z(j2+1) &
                    + CS(4,  3,i)*U(L)  + CS(5,  3,i)*U(L+1) + CS(6,  3,i)*U(L+2)
         ...
         call vnorm( U, 3*jx )
!c                    corner #3
         Z(i0)   = AC(1,1,3,i)*Z(j0) + AC(2,1,3,i)*Z(j0+1) + AC(3,1,3,i)*Z(j1) + AC(4,1,3,i)*Z(j1+1) + CC(1,  3,i)*U(1)
         Z(i0+1) = AC(1,2,3,i)*Z(j0) + AC(2,2,3,i)*Z(j0+1) + AC(3,2,3,i)*Z(j1) + AC(4,2,3,i)*Z(j1+1) + CC(2,  3,i)*U(1)  + CC(3,  3,i)*U(2)
         Z(i1)   = AC(1,3,3,i)*Z(j0) + AC(2,3,3,i)*Z(j0+1) + AC(3,3,3,i)*Z(j1) + AC(4,3,3,i)*Z(j1+1) + CC(4,  3,i)*U(1)  + CC(5,  3,i)*U(2) + CC(6,  3,i)*U(3)
         Z(i1+1) = four*Z(j1) - Z(i0) - Z(i0+1) - Z(i1)
!c                    side #4
         L = 4
         do js = 1, jx-2
            i0 = i0 + 2
            i1 = i1 + 2
            Z(i0)   = AS(1,1,4,i)*Z(j0  ) + AS(2,1,4,i)*Z(j0+1) + AS(3,1,4,i)*Z(j0+2) + AS(4,1,4,i)*Z(j1) + AS(5,1,4,i)*Z(j1+1) + AS(6,1,4,i)*Z(j1+2)+ CS(1,  4,i)*U(L)
            Z(i0+1) = AS(1,2,4,i)*Z(j0  ) + AS(2,2,4,i)*Z(j0+1) + AS(3,2,4,i)*Z(j0+2) + AS(4,2,4,i)*Z(j1) + AS(5,2,4,i)*Z(j1+1) + AS(6,2,4,i)*Z(j1+2) &
                    + CS(2,  4,i)*U(L)    + CS(3,  4,i)*U(L+1)
            Z(i1)   = AS(1,3,4,i)*Z(j0  ) + AS(2,3,4,i)*Z(j0+1) + AS(3,3,4,i)*Z(j0+2) + AS(4,3,4,i)*Z(j1) + AS(5,3,4,i)*Z(j1+1) + AS(6,3,4,i)*Z(j1+2) &
                    + CS(4,  4,i)*U(L)    + CS(5,  4,i)*U(L+1) + CS(6,  4,i)*U(L+2)
            Z(i1+1) = four*Z(j1+1) - Z(i0) - Z(i0+1) - Z(i1)
            j0 = j0 + 1
            j1 = j1 + 1
            L  = L  + 3
...
!c                    corner #4
         i0 = i0 + 2
         i1 = i1 + 2
         Z(i0)   = AC(1,1,4,i)*Z(j0) + AC(2,1,4,i)*Z(j0+1) + AC(3,1,4,i)*Z(j1) + AC(4,1,4,i)*Z(j1+1) + CC(1,  4,i)*U(L)
         Z(i0+1) = AC(1,2,4,i)*Z(j0) + AC(2,2,4,i)*Z(j0+1) + AC(3,2,4,i)*Z(j1) + AC(4,2,4,i)*Z(j1+1) + CC(2,  4,i)*U(L)  + CC(3,  4,i)*U(L+1)
         Z(i1)   = AC(1,3,4,i)*Z(j0) + AC(2,3,4,i)*Z(j0+1) + AC(3,3,4,i)*Z(j1) + AC(4,3,4,i)*Z(j1+1) + CC(4,  4,i)*U(L)  + CC(5,  4,i)*U(L+1) + CC(6,  4,i)*U(L+2)
         Z(i1+1) = four*Z(j1+1) - Z(i0) - Z(i0+1) - Z(i1)
