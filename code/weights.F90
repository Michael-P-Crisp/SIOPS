...
subroutine getweights(esize,srad,nzef,prad,npdepths,pdepths,cg_tol,cg_limit,startstress,soilweight_out)
...
!-----------------------input and initialisation--------------------------
 character(1000) str2
 integer temp_prad(2) ! current width of the foundation (dynamic for pad footings)
 integer temp_pdepths ! actual depth of footing
...
do pd = 1,npdepths
    ...
    if (pile_foundation) then
        temp_prad = prad
        temp_pdepths = pdepths(pd)
        nxp = srad
        nyp = srad
    else
        temp_prad = pdepths(pd)
        temp_pdepths = 0
        ! get true corner of current foundation
        nxp = srad + floor(real(prad(1))/2) - floor(real(temp_prad(1))/2)
        nyp = srad + floor(real(prad(1))/2) - floor(real(temp_prad(1))/2)
    end if
