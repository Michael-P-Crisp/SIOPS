...
!c---------------------------------------------------------------------------
      real*8 function dcvaa3(cov,Dx,Dy,Dz,C1,C2,C3,var,pb,dthx,dthy,dthz, H, G, F)
      parameter (NG = 20)
      implicit real*8 (a-h,o-z)
      dimension w(NG), z(NG)
      real(8), intent(in) :: var, pb, dthx, dthy, dthz
      real(8)  H, G, F
      ...
      do i = 1, NG
         ...
         do j = 1, NG
            ...
            do k = 1, NG
               ...
               dp  = one + z(k)
               dm  = one - z(k)
               d31 = d31 + w(k)*(dp*cov(x1,y1,z1,var, pb, dthx,dthy, dthz, H, G, F) + dm*cov(x1,y1,z2,var, pb, dthx, dthy, dthz, H, G, F))
               d32 = d32 + w(k)*(dp*cov(x1,y2,z1,var, pb, dthx,dthy, dthz, H, G, F) + dm*cov(x1,y2,z2,var, pb, dthx, dthy, dthz, H, G, F))
               d33 = d33 + w(k)*(dp*cov(x2,y1,z1,var, pb, dthx,dthy, dthz, H, G, F) + dm*cov(x2,y1,z2,var, pb, dthx, dthy, dthz, H, G, F))
               d34 = d34 + w(k)*(dp*cov(x2,y2,z1,var, pb, dthx,dthy, dthz, H, G, F) + dm*cov(x2,y2,z2,var, pb, dthx, dthy, dthz, H, G, F))
            end do
            dp  = one + z(j)
            dm  = one - z(j)
