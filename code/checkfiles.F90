...
write(unit,*) "1		!read_si parameter. 1 = Generate investigations from the below variables. 2 = read investigation configurations from 'input/si.txt' data. 3 = also read tests and depths on a per-borehole basis."
...
write(unit,*) "4096,	6					!limit for stage 0 matrix size, where this value is k1*k2*k3, max no. subdivisions"
