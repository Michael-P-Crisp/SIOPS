...
!c  boundary).
!c
!c  For more information on local average processes, see
!c
...
!	4.0 	Updated syntax to fortran 90 standard.
!			Moved common block variables into subroutine arguments for extra safety.
!			Limiting parameters MXM,MXK,NGS are now flexible at run time, removing limits.
!			Initialisation of correlation matrices is now handled by a seperate subroutine
!				for cleanliness when interfacing with other languages.
!			(May 2016 Michael Crisp)
!c ==========================================================================


      subroutine las3g( Z, N1, N2, N3, XL, YL, ZL, kseed,iout  &
                   ,C0, CC, CE, CS, CI, AC, AE, AS, AI,ATC, ATS, ATI, CTC, CTS, CTI,M, k1, k2, k3, kk, MXM,MXK,NGS)
 
      
      !parameter( NGS = 32000 )
      integer, intent(in) :: MXM,MXK,NGS
      real, intent(out) :: Z(*)
      !real(8) dvar, dpb, dthx, dthy, dthz
      real(8) H,G,F !dummy arguments for nested subroutines, not sure where needed
      ...
      real(8) :: dvfn
      external :: dvfn
      real, external :: dot1, dot2, dot3, dot4, dot5, dot6, dot7
      common/LASTYM/ ti, ts
...
!c					generate stage 0 field
	!MAKE SURE VNORM USES KSEED TO ENSURE IT'S NOT RANDOM EVERY TIME!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !Or otherwise initialize it first (might be safer since variabes are saved)
      call vnorm( U, kk )
      L = 1
      do i = 1, kk
         Z(iq+i) = C0(L)*U(1)
...
!c								corner #1
         ...
         Z(i3+1) = eight*Z(j0) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
!c								edge #1
         L = 8
         do js = 1, jx-2
          i0 = i0 + 2
          ...
          Z(i3+1)=eight*Z(j0+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
          j0 = j0 + 1
          j1 = j1 + 1
          j3 = j3 + 1
          j4 = j4 + 1
...
!c								corner #2
         ...
         do ks = 1, jy-2
            ...
            call vnorm( U, 7*jx )
!c								edge #2
            Z(i0)   = dot12v(Z,AE(1,1,2,i),CE( 1,2,i),U(1),dot1,j0,j1,j2,j3,j4,j5)
            Z(i0+1) = dot12v(Z,AE(1,2,2,i),CE( 2,2,i),U(1),dot2,j0,j1,j2,j3,j4,j5)
            Z(i1)   = dot12v(Z,AE(1,3,2,i),CE( 4,2,i),U(1),dot3,j0,j1,j2,j3,j4,j5)
            Z(i1+1) = dot12v(Z,AE(1,4,2,i),CE( 7,2,i),U(1),dot4,j0,j1,j2,j3,j4,j5)
            Z(i2)   = dot12v(Z,AE(1,5,2,i),CE(11,2,i),U(1),dot5,j0,j1,j2,j3,j4,j5)
            Z(i2+1) = dot12v(Z,AE(1,6,2,i),CE(16,2,i),U(1),dot6,j0,j1,j2,j3,j4,j5)
            Z(i3)   = dot12v(Z,AE(1,7,2,i),CE(22,2,i),U(1),dot7,j0,j1,j2,j3,j4,j5)
            Z(i3+1) = eight*Z(j1  ) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
!c								side #1
            L = 8
            do js = 1, jx-2
               i0 = i0 + 2
               ...
               i2 = i2 + 2
               i3 = i3 + 2
               Z(i0  ) = dot18a(Z,AS(1,1,1,i),CS( 1,1,i),U(L),dot1,j0,j1,j2,j3,j4,j5)
               Z(i0+1) = dot18a(Z,AS(1,2,1,i),CS( 2,1,i),U(L),dot2,j0,j1,j2,j3,j4,j5)
               Z(i1  ) = dot18a(Z,AS(1,3,1,i),CS( 4,1,i),U(L),dot3,j0,j1,j2,j3,j4,j5)
               Z(i1+1) = dot18a(Z,AS(1,4,1,i),CS( 7,1,i),U(L),dot4,j0,j1,j2,j3,j4,j5)
               Z(i2  ) = dot18a(Z,AS(1,5,1,i),CS(11,1,i),U(L),dot5,j0,j1,j2,j3,j4,j5)
               Z(i2+1) = dot18a(Z,AS(1,6,1,i),CS(16,1,i),U(L),dot6,j0,j1,j2,j3,j4,j5)
               Z(i3  ) = dot18a(Z,AS(1,7,1,i),CS(22,1,i),U(L),dot7,j0,j1,j2,j3,j4,j5)
               Z(i3+1) = eight*Z(j1+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								edge #3
            ...
            i2 = i2 + 2
            i3 = i3 + 2
            Z(i0)   = dot12v(Z,AE(1,1,3,i),CE( 1,3,i),U(L),dot1,j0,j1,j2,j3,j4,j5)
            Z(i0+1) = dot12v(Z,AE(1,2,3,i),CE( 2,3,i),U(L),dot2,j0,j1,j2,j3,j4,j5)
            Z(i1)   = dot12v(Z,AE(1,3,3,i),CE( 4,3,i),U(L),dot3,j0,j1,j2,j3,j4,j5)
            Z(i1+1) = dot12v(Z,AE(1,4,3,i),CE( 7,3,i),U(L),dot4,j0,j1,j2,j3,j4,j5)
            Z(i2)   = dot12v(Z,AE(1,5,3,i),CE(11,3,i),U(L),dot5,j0,j1,j2,j3,j4,j5)
            Z(i2+1) = dot12v(Z,AE(1,6,3,i),CE(16,3,i),U(L),dot6,j0,j1,j2,j3,j4,j5)
            Z(i3)   = dot12v(Z,AE(1,7,3,i),CE(22,3,i),U(L),dot7,j0,j1,j2,j3,j4,j5)
            Z(i3+1) = eight*Z(j1+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								corner #4
         ...
         do ls = 1, jz-2
            ...
            call vnorm( U, 7*jx )
!c								edge #5
            Z(i0)   = dot12v(Z,AE(1,1,5,i),CE( 1,5,i),U(1),dot1,j0,j1,j3,j4,j6,j7)
            Z(i0+1) = dot12v(Z,AE(1,2,5,i),CE( 2,5,i),U(1),dot2,j0,j1,j3,j4,j6,j7)
            Z(i1)   = dot12v(Z,AE(1,3,5,i),CE( 4,5,i),U(1),dot3,j0,j1,j3,j4,j6,j7)
            Z(i1+1) = dot12v(Z,AE(1,4,5,i),CE( 7,5,i),U(1),dot4,j0,j1,j3,j4,j6,j7)
            Z(i2)   = dot12v(Z,AE(1,5,5,i),CE(11,5,i),U(1),dot5,j0,j1,j3,j4,j6,j7)
            Z(i2+1) = dot12v(Z,AE(1,6,5,i),CE(16,5,i),U(1),dot6,j0,j1,j3,j4,j6,j7)
            Z(i3)   = dot12v(Z,AE(1,7,5,i),CE(22,5,i),U(1),dot7,j0,j1,j3,j4,j6,j7)
            Z(i3+1) = eight*Z(j3  ) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
!c								side #2
            L = 8
            do js = 1, jx-2
               i0 = i0 + 2
               ...
               i2 = i2 + 2
               i3 = i3 + 2
               Z(i0  ) = dot18a(Z,AS(1,1,2,i),CS( 1,2,i),U(L),dot1,j0,j1,j3,j4,j6,j7)
               Z(i0+1) = dot18a(Z,AS(1,2,2,i),CS( 2,2,i),U(L),dot2,j0,j1,j3,j4,j6,j7)
               Z(i1  ) = dot18a(Z,AS(1,3,2,i),CS( 4,2,i),U(L),dot3,j0,j1,j3,j4,j6,j7)
               Z(i1+1) = dot18a(Z,AS(1,4,2,i),CS( 7,2,i),U(L),dot4,j0,j1,j3,j4,j6,j7)
               Z(i2  ) = dot18a(Z,AS(1,5,2,i),CS(11,2,i),U(L),dot5,j0,j1,j3,j4,j6,j7)
               Z(i2+1) = dot18a(Z,AS(1,6,2,i),CS(16,2,i),U(L),dot6,j0,j1,j3,j4,j6,j7)
               Z(i3  ) = dot18a(Z,AS(1,7,2,i),CS(22,2,i),U(L),dot7,j0,j1,j3,j4,j6,j7)
               Z(i3+1) = eight*Z(j3+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								edge #6
            ...
            i2 = i2 + 2
            i3 = i3 + 2
            Z(i0)   = dot12v(Z,AE(1,1,6,i),CE( 1,6,i),U(L),dot1,j0,j1,j3,j4,j6,j7)
            Z(i0+1) = dot12v(Z,AE(1,2,6,i),CE( 2,6,i),U(L),dot2,j0,j1,j3,j4,j6,j7)
            Z(i1)   = dot12v(Z,AE(1,3,6,i),CE( 4,6,i),U(L),dot3,j0,j1,j3,j4,j6,j7)
            Z(i1+1) = dot12v(Z,AE(1,4,6,i),CE( 7,6,i),U(L),dot4,j0,j1,j3,j4,j6,j7)
            Z(i2)   = dot12v(Z,AE(1,5,6,i),CE(11,6,i),U(L),dot5,j0,j1,j3,j4,j6,j7)
            Z(i2+1) = dot12v(Z,AE(1,6,6,i),CE(16,6,i),U(L),dot6,j0,j1,j3,j4,j6,j7)
            Z(i3)   = dot12v(Z,AE(1,7,6,i),CE(22,6,i),U(L),dot7,j0,j1,j3,j4,j6,j7)
            Z(i3+1) = eight*Z(j3+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
            ...
            do ks = 1, jy-2
               ...
               call vnorm( U, 7*jx )
!c								side #3
               Z(i0  ) = dot18s(Z,AS(1,1,3,i),CS( 1,3,i),U(1),dot1,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i0+1) = dot18s(Z,AS(1,2,3,i),CS( 2,3,i),U(1),dot2,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i1  ) = dot18s(Z,AS(1,3,3,i),CS( 4,3,i),U(1),dot3,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i1+1) = dot18s(Z,AS(1,4,3,i),CS( 7,3,i),U(1),dot4,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i2  ) = dot18s(Z,AS(1,5,3,i),CS(11,3,i),U(1),dot5,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i2+1) = dot18s(Z,AS(1,6,3,i),CS(16,3,i),U(1),dot6,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i3  ) = dot18s(Z,AS(1,7,3,i),CS(22,3,i),U(1),dot7,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i3+1) = eight*Z(j4  ) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
               ...
               do js = 1, jx-2
                  ...
                  i3 = i3 + 2
!c								interior
                  Z(i0  ) = dot27i(Z,AI(1,1,i),CI( 1,i),U(L),dot1,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i0+1) = dot27i(Z,AI(1,2,i),CI( 2,i),U(L),dot2,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i1  ) = dot27i(Z,AI(1,3,i),CI( 4,i),U(L),dot3,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i1+1) = dot27i(Z,AI(1,4,i),CI( 7,i),U(L),dot4,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i2  ) = dot27i(Z,AI(1,5,i),CI(11,i),U(L),dot5,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i2+1) = dot27i(Z,AI(1,6,i),CI(16,i),U(L),dot6,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i3  ) = dot27i(Z,AI(1,7,i),CI(22,i),U(L),dot7,j0,j1,j2,j3,j4,j5,j6,j7,j8)
                  Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
                  j0 = j0 + 1
                  j1 = j1 + 1
                  j2 = j2 + 1
                  j3 = j3 + 1
...
!c								side #4
               ...
               i2 = i2 + 2
               i3 = i3 + 2
               Z(i0  ) = dot18s(Z,AS(1,1,4,i),CS( 1,4,i),U(L),dot1,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i0+1) = dot18s(Z,AS(1,2,4,i),CS( 2,4,i),U(L),dot2,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i1  ) = dot18s(Z,AS(1,3,4,i),CS( 4,4,i),U(L),dot3,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i1+1) = dot18s(Z,AS(1,4,4,i),CS( 7,4,i),U(L),dot4,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i2  ) = dot18s(Z,AS(1,5,4,i),CS(11,4,i),U(L),dot5,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i2+1) = dot18s(Z,AS(1,6,4,i),CS(16,4,i),U(L),dot6,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i3  ) = dot18s(Z,AS(1,7,4,i),CS(22,4,i),U(L),dot7,j0,j1,j2,j3,j4,j5,j6,j7,j8)
               Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
               j0 = j0 + 2
            end do
            j1 = j0 + jx
            ...
            call vnorm( U, 7*jx )
!c								edge #7
            Z(i0)   = dot12v(Z,AE(1,1,7,i),CE( 1,7,i),U(1),dot1,j0,j1,j3,j4,j6,j7)
            Z(i0+1) = dot12v(Z,AE(1,2,7,i),CE( 2,7,i),U(1),dot2,j0,j1,j3,j4,j6,j7)
            Z(i1)   = dot12v(Z,AE(1,3,7,i),CE( 4,7,i),U(1),dot3,j0,j1,j3,j4,j6,j7)
            Z(i1+1) = dot12v(Z,AE(1,4,7,i),CE( 7,7,i),U(1),dot4,j0,j1,j3,j4,j6,j7)
            Z(i2)   = dot12v(Z,AE(1,5,7,i),CE(11,7,i),U(1),dot5,j0,j1,j3,j4,j6,j7)
            Z(i2+1) = dot12v(Z,AE(1,6,7,i),CE(16,7,i),U(1),dot6,j0,j1,j3,j4,j6,j7)
            Z(i3)   = dot12v(Z,AE(1,7,7,i),CE(22,7,i),U(1),dot7,j0,j1,j3,j4,j6,j7)
            Z(i3+1) = eight*Z(j4  ) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
!c								side #5
            L = 8
            do js = 1, jx-2
               ...
               i2 = i2 + 2
               i3 = i3 + 2
               Z(i0  ) = dot18a(Z,AS(1,1,5,i),CS( 1,5,i),U(L),dot1,j0,j1,j3,j4,j6,j7)
               Z(i0+1) = dot18a(Z,AS(1,2,5,i),CS( 2,5,i),U(L),dot2,j0,j1,j3,j4,j6,j7)
               Z(i1  ) = dot18a(Z,AS(1,3,5,i),CS( 4,5,i),U(L),dot3,j0,j1,j3,j4,j6,j7)
               Z(i1+1) = dot18a(Z,AS(1,4,5,i),CS( 7,5,i),U(L),dot4,j0,j1,j3,j4,j6,j7)
               Z(i2  ) = dot18a(Z,AS(1,5,5,i),CS(11,5,i),U(L),dot5,j0,j1,j3,j4,j6,j7)
               Z(i2+1) = dot18a(Z,AS(1,6,5,i),CS(16,5,i),U(L),dot6,j0,j1,j3,j4,j6,j7)
               Z(i3  ) = dot18a(Z,AS(1,7,5,i),CS(22,5,i),U(L),dot7,j0,j1,j3,j4,j6,j7)
               Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								edge #8
            ...
            i2 = i2 + 2
            i3 = i3 + 2
            Z(i0)   = dot12v(Z,AE(1,1,8,i),CE( 1,8,i),U(L),dot1,j0,j1,j3,j4,j6,j7)
            Z(i0+1) = dot12v(Z,AE(1,2,8,i),CE( 2,8,i),U(L),dot2,j0,j1,j3,j4,j6,j7)
            Z(i1)   = dot12v(Z,AE(1,3,8,i),CE( 4,8,i),U(L),dot3,j0,j1,j3,j4,j6,j7)
            Z(i1+1) = dot12v(Z,AE(1,4,8,i),CE( 7,8,i),U(L),dot4,j0,j1,j3,j4,j6,j7)
            Z(i2)   = dot12v(Z,AE(1,5,8,i),CE(11,8,i),U(L),dot5,j0,j1,j3,j4,j6,j7)
            Z(i2+1) = dot12v(Z,AE(1,6,8,i),CE(16,8,i),U(L),dot6,j0,j1,j3,j4,j6,j7)
            Z(i3)   = dot12v(Z,AE(1,7,8,i),CE(22,8,i),U(L),dot7,j0,j1,j3,j4,j6,j7)
            Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								corner #6
         ...
         do ks = 1, jy-2
            ...
            call vnorm( U, 7*jx )
!c								edge #10
            Z(i0)   = dot12v(Z,AE(1,1,10,i),CE( 1,10,i),U(1),dot1,j0,j1,j2,j3,j4,j5)
            Z(i0+1) = dot12v(Z,AE(1,2,10,i),CE( 2,10,i),U(1),dot2,j0,j1,j2,j3,j4,j5)
            Z(i1)   = dot12v(Z,AE(1,3,10,i),CE( 4,10,i),U(1),dot3,j0,j1,j2,j3,j4,j5)
            Z(i1+1) = dot12v(Z,AE(1,4,10,i),CE( 7,10,i),U(1),dot4,j0,j1,j2,j3,j4,j5)
            Z(i2)   = dot12v(Z,AE(1,5,10,i),CE(11,10,i),U(1),dot5,j0,j1,j2,j3,j4,j5)
            Z(i2+1) = dot12v(Z,AE(1,6,10,i),CE(16,10,i),U(1),dot6,j0,j1,j2,j3,j4,j5)
            Z(i3)   = dot12v(Z,AE(1,7,10,i),CE(22,10,i),U(1),dot7,j0,j1,j2,j3,j4,j5)
            Z(i3+1) = eight*Z(j4  ) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
!c								side #6
            L = 8
            do js = 1, jx-2
               ...
               i2 = i2 + 2
               i3 = i3 + 2
               Z(i0  ) = dot18a(Z,AS(1,1,6,i),CS( 1,6,i),U(L),dot1,j0,j1,j2,j3,j4,j5)
               Z(i0+1) = dot18a(Z,AS(1,2,6,i),CS( 2,6,i),U(L),dot2,j0,j1,j2,j3,j4,j5)
               Z(i1  ) = dot18a(Z,AS(1,3,6,i),CS( 4,6,i),U(L),dot3,j0,j1,j2,j3,j4,j5)
               Z(i1+1) = dot18a(Z,AS(1,4,6,i),CS( 7,6,i),U(L),dot4,j0,j1,j2,j3,j4,j5)
               Z(i2  ) = dot18a(Z,AS(1,5,6,i),CS(11,6,i),U(L),dot5,j0,j1,j2,j3,j4,j5)
               Z(i2+1) = dot18a(Z,AS(1,6,6,i),CS(16,6,i),U(L),dot6,j0,j1,j2,j3,j4,j5)
               Z(i3  ) = dot18a(Z,AS(1,7,6,i),CS(22,6,i),U(L),dot7,j0,j1,j2,j3,j4,j5)
               Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								edge #11
            ...
            i2 = i2 + 2
            i3 = i3 + 2
            Z(i0)   = dot12v(Z,AE(1,1,11,i),CE( 1,11,i),U(L),dot1,j0,j1,j2,j3,j4,j5)
            Z(i0+1) = dot12v(Z,AE(1,2,11,i),CE( 2,11,i),U(L),dot2,j0,j1,j2,j3,j4,j5)
            Z(i1)   = dot12v(Z,AE(1,3,11,i),CE( 4,11,i),U(L),dot3,j0,j1,j2,j3,j4,j5)
            Z(i1+1) = dot12v(Z,AE(1,4,11,i),CE( 7,11,i),U(L),dot4,j0,j1,j2,j3,j4,j5)
            Z(i2)   = dot12v(Z,AE(1,5,11,i),CE(11,11,i),U(L),dot5,j0,j1,j2,j3,j4,j5)
            Z(i2+1) = dot12v(Z,AE(1,6,11,i),CE(16,11,i),U(L),dot6,j0,j1,j2,j3,j4,j5)
            Z(i3)   = dot12v(Z,AE(1,7,11,i),CE(22,11,i),U(L),dot7,j0,j1,j2,j3,j4,j5)
            Z(i3+1) = eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
...
!c								edge #12
         ...
         do js = 1, jx-2
          ...
          i2 = i2 + 2
          i3 = i3 + 2
          Z(i0)  =dot12h(Z,AE(1,1,12,i),CE( 1,12,i),U(L),dot1,j0,j1,j3,j4)
          Z(i0+1)=dot12h(Z,AE(1,2,12,i),CE( 2,12,i),U(L),dot2,j0,j1,j3,j4)
          Z(i1)  =dot12h(Z,AE(1,3,12,i),CE( 4,12,i),U(L),dot3,j0,j1,j3,j4)
          Z(i1+1)=dot12h(Z,AE(1,4,12,i),CE( 7,12,i),U(L),dot4,j0,j1,j3,j4)
          Z(i2)  =dot12h(Z,AE(1,5,12,i),CE(11,12,i),U(L),dot5,j0,j1,j3,j4)
          Z(i2+1)=dot12h(Z,AE(1,6,12,i),CE(16,12,i),U(L),dot6,j0,j1,j3,j4)
          Z(i3)  =dot12h(Z,AE(1,7,12,i),CE(22,12,i),U(L),dot7,j0,j1,j3,j4)
          Z(i3+1)=eight*Z(j4+1) - Z(i0) - Z(i0+1) - Z(i1) - Z(i1+1) - Z(i2) - Z(i2+1) - Z(i3)
          j0 = j0 + 1
          j1 = j1 + 1
          j3 = j3 + 1
