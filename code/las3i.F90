...
!c----------------------------------------------------------------------------
      subroutine las3i( dvfn, N1, N2, N3, XL, YL, ZL, MXM, MXK, C0, CC, CE, CS, CI, AC, AE, AS, AI, &
      ...
      integer, intent(out) :: k1,k2,k3,kk,M
      integer j1,j2,j3
...
!20    if(  kk .gt. MXK ) go to 15
      ...
      do m = 0, MXM
         ...
         j2 = k2/2
         j3 = k3/2
         if( 2*j1 .ne. k1 .or. 2*j2 .ne. k2 .or. 2*j3 .ne. k3 ) exit
         k1 = j1
         k2 = j2
         k3 = j3
      end do
	...
	if(  kk .gt. MXK ) then
      write(*,*)'Error: unable to determine an acceptable combination of k1, k2, k3 and m'
      write(*,*)'       such that k1*2**m = N1, k2*2**m = N2 and k3*2**m = N3.'
      write(*,*)'       k1 = ',k1,', k2 = ',k2,', k3 = ',k3,', m = ',m
      write(*,*)'       (k1*k2*k3 must be less than ',MXK,' and m must be less than ',MXM,')'
      write(*,*)'       Try changing N1, N2, and/or N3.'
      stop
    end if
...
!c						initialize internal generator (this is now instead done in las2g).
!      kseed = iseed( kseed )
!c						form initial covariance matrix
      T1 = dble(XL)/dble(k1)
      T2 = dble(YL)/dble(k2)
      T3 = dble(ZL)/dble(k3)
      call dcvit3( dvfn, R0, kk, R, 27, k1, k2, k3, T1, T2, T3, dvar, dpb, dthx, dthy, dthz , H, G, F)
...
!						(special k1,k2,k3 = 1 case)
      ...
      if( (lk1 .or. lk2 .or. lk3) .and. (m .gt. 0) ) then
         ...
         T3 = 0.5d0*T3
         lformR = (1 .lt. m)
         call dcvmt3(dvfn,R(1,1,in),27,B,8,S,27,T1,T2,T3,lformR,dvar, dpb, dthx, dthy, dthz, H, G, F)
...
!c						this should not happen...
            ...
            call thin1d(R(1,1,io),27,B,8,S,27,ATS,6,ATI,9,CTS,CTI,i1,i2,i3,7,iout,tol)
         else
            if( lk1 ) k = 1
            if( lk2 ) k = 2
            if( lk3 ) k = 3
            call corn2d(R(1,1,io),27,B,8,S,27,CTC,28,7,ATC,mc(1,1,k),iout,tol)

            call side2d(R(1,1,io),27,B,8,S,27,CTS,28,7,ATS,ms(1,1,k),iout,tol)

            call intr2d(R(1,1,io),27,B,8,S,27,CTI,7,ATI,mi(1,k),iout,tol)
         endif
         in = 1
         io = 2
         nm = 2
...
!c							corner parameters
         ...
         call corn3d(R(1,1,io),27,B,8,S,27,CC(1,1,k),AC(1,1,1,k),iout,tol)
!c							edge parameters

         call edge3d(R(1,1,io),27,B,8,S,27,CE(1,1,k),AE(1,1,1,k),iout,tol)
!c							side parameters

         call side3d(R(1,1,io),27,B,8,S,27,CS(1,1,k),AS(1,1,1,k),iout,tol)
!							interior parameters

         call intr3d(R(1,1,io),27,B,8,S,27,CI(1,k),AI(1,1,k),iout,tol)
