...
!   Modified by Michael Crisp August 18 2016
!c
...
!
!       Version 1.1 - converted to f95 format to make it easier to modify
!                   - Pass in variables through arguments rather than common
!c----------------------------------------------------------------------------
      subroutine dcvmt3( dvfn, R, ir, B, ib, S, is, Dx, Dy, Dz, lformR, dvar, dpb, dthx, dthy, dthz, H, G, F)
      implicit real*8 (a-h,o-z)
      dimension R(ir,*), B(ib,*), S(is,*)
      dimension T(3)
      integer map(5)
      logical lformR
      real(8) dvar, dpb, dthx, dthy, dthz !variables to pass through to other subroutines
      real(8) H,G,F
      external dvfn
...
!					first find B's essential elements
      B(1,1) = dcvaa3(dvfn,Dx,Dy,Dz,zero,zero,zero,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,2) = dcvaa3(dvfn,Dx,Dy,Dz, one,zero,zero,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,3) = dcvaa3(dvfn,Dx,Dy,Dz,zero, one,zero,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,4) = dcvaa3(dvfn,Dx,Dy,Dz, one, one,zero,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,5) = dcvaa3(dvfn,Dx,Dy,Dz,zero,zero, one,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,6) = dcvaa3(dvfn,Dx,Dy,Dz, one,zero, one,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,7) = dcvaa3(dvfn,Dx,Dy,Dz,zero, one, one,dvar, dpb, dthx, dthy,dthz, H, G, F)
      B(1,8) = dcvaa3(dvfn,Dx,Dy,Dz, one, one, one,dvar, dpb, dthx, dthy,dthz, H, G, F)
...
!						essential elements first ...
         R(1, 1) = B(1,1)
         R(1, 2) = B(1,2)
         R(1, 3) = dcvaa3(dvfn,Dx,Dy,Dz,  two, zero, zero ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1, 4) = B(1,3)
         R(1, 5) = B(1,4)
         R(1, 6) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  one, zero ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1, 7) = dcvaa3(dvfn,Dx,Dy,Dz, zero,  two, zero ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1, 8) = dcvaa3(dvfn,Dx,Dy,Dz,  one,  two, zero ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1, 9) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  two, zero ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,10) = B(1,5)
         R(1,11) = B(1,6)
         R(1,12) = dcvaa3(dvfn,Dx,Dy,Dz,  two, zero,  one ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,13) = B(1,7)
         R(1,14) = B(1,8)
         R(1,15) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  one,  one ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,16) = dcvaa3(dvfn,Dx,Dy,Dz, zero,  two,  one ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,17) = dcvaa3(dvfn,Dx,Dy,Dz,  one,  two,  one ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,18) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  two,  one ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,19) = dcvaa3(dvfn,Dx,Dy,Dz, zero, zero,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,20) = dcvaa3(dvfn,Dx,Dy,Dz,  one, zero,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,21) = dcvaa3(dvfn,Dx,Dy,Dz,  two, zero,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,22) = dcvaa3(dvfn,Dx,Dy,Dz, zero,  one,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,23) = dcvaa3(dvfn,Dx,Dy,Dz,  one,  one,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,24) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  one,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,25) = dcvaa3(dvfn,Dx,Dy,Dz, zero,  two,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,26) = dcvaa3(dvfn,Dx,Dy,Dz,  one,  two,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
         R(1,27) = dcvaa3(dvfn,Dx,Dy,Dz,  two,  two,  two ,dvar, dpb, dthx, dthy,dthz, H, G, F)
...
!					now find S (essential elements first)
      ...
      do k = 1, 3
          do j = 1, 3
             S(ii,  1) = dcvab3(dvfn,Dx,Dy,Dz,onept5,T(j),T(k),dvar, dpb, dthx, dthy,dthz, H, G, F)
             S(ii+1,1) = dcvab3(dvfn,Dx,Dy,Dz,half,  T(j),T(k),dvar, dpb, dthx, dthy,dthz, H, G, F)
             S(ii+2,1) = dcvab3(dvfn,Dx,Dy,Dz,twopt5,T(j),T(k),dvar, dpb, dthx, dthy,dthz, H, G, F)
             ii = ii + 3
          end do
      end do
